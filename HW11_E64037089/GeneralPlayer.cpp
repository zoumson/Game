...
void GeneralPlayer::setAttr(int l) // automatically calculate attributes for a given level
{
    ...
    backpack_weight; // current weight of player’s backpack
    backpack_slot_limit = (int)sqrt(level+3); // limit on number of backpack’s slots
    backpack_weight_limit = 50*backpack_slot_limit; // limit on weight of backpack in gram
    weapon[(int)sqrt(level+2)]; // weapon item
    armor[backpack_slot_limit - (int)sqrt(level+2)]; // armor item
    
    /*
    int backpackX = level+3;
    int backpackY = level+3;
    backpack = new Item*[backpackX];
    for(int i = 0; i < backpackX; ++i)
       backpack[i] = new Item[backpackY];//problem here
    
    For unused slots of backpack, please assign them as NULL pointer
    for(int i = 0; i < backpackX ; i++)
    {
         for(int j = 0; i < backpackY ; j++)
        { backpack[i][j] = NULL; }
    ...
    int occupied_slot = 0;
    for(int i = 0; i < slot_limit; i++) {      if(backpack[i] != NULL)          occupied_slot++;  }
